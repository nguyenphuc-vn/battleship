...
public class Game {
    public boolean checkGrid(List<List> grid, String input, String name, int length) {
    ...
    private void addRow(List<List> grid, int pos1, int pos2, int pos) {
        for (int col = pos1; col <= pos2; col++) {
            grid.get(pos).set(col, 'O');
        }
    }

    private void addCol(List<List> grid, int pos1, int pos2, int pos) {
        for (int col = pos1; col <= pos2; col++) {
    ...
    private boolean isOccupied(List<List> list, int row1, int col1, int row2, int col2) {
        for (int row = row1 - 1; row <= row2 + 1; row++) {
            for (int col = col1 - 1; col <= col2 + 1; col++) {
                if (list.get(row).get(col).equals('O')) {
    ...
    /*Check shot hits*/
    public boolean placeSymbol(List<List> grid, String input, Engine engine,List<List> gridState) {
        String[] inputs = input.split("\\s");
        int row = parse(inputs[0]);
        int col = parse(inputs[1]);
        if (grid.get(row).get(col).equals('O')) {
            grid.get(row).set(col, 'X');
            gridState.get(row).set(col,'X');
            engine.printGrid(gridState);
            System.out.println("You hit a ship!");
            engine.printGrid(grid);
            return true;
        }
        grid.get(row).set(col, 'M');
        gridState.get(row).set(col, 'M');
        engine.printGrid(gridState);
